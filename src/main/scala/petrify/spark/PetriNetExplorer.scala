...
object PetriNetExplorer {
  def lookForHelper(petriNet: PetriNet, pastStates: RDD[State], toIterate: RDD[State], end: State => Boolean):Boolean = {
  ...
  def lookFor(sc: SparkContext, petriNet: PetriNet, start: State, end: State => Boolean):Boolean = {
